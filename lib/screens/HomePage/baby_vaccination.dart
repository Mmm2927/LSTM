...
class _BabyVaccination extends State<BabyVaccination> {
  ...
  void initState() {
    getVaccinateList();
    getMyVaccineFuture = getMyVaccine();
    // 개월 수에 따라 선택 하도록!
  ...
  getVaccinateList(){
    ...
    // id :
    vaccines1 = {
      '0M': getMonthVaccines([0,1,2]),
      '1M': getMonthVaccines([2]),
      '2M': getMonthVaccines([4,5,6,7,8,9]),
      '4M': getMonthVaccines([10,11,12,13,14,15]),
      '6M': getMonthVaccines([16,17,18,19,20,21,22,23]),
    };
    vaccines2 = {
      '12M' : getMonthVaccines([24, 25, 26, 27, 29,30, 31, 32, 34,35]),
      '15M' : getMonthVaccines([24, 25, 26, 27, 28, 29,30, 31, 32, 34,35]),
      '18M' : getMonthVaccines([28, 29, 30, 31, 32,34,35]),
      '23M' : getMonthVaccines([29, 30, 31, 32, 34,35,36]),
      '35M' : getMonthVaccines([33, 34,35,37]),
    };
    vaccines3 = {
      '4세' : getMonthVaccines([38, 40, 41]),
      '6세' : getMonthVaccines([38, 40, 41, 42]),
      '11세': getMonthVaccines([39]),
      '12세': getMonthVaccines([39,43,44]),
    };
    vaccinesAll = [vaccines1, vaccines2, vaccines3];
  }
  List<Widget> getMonthVaccines(List<int> nums){
    List<Widget> tmp = [];
            ...
            children: [
              ...
              Row(
                children: const [
                  Icon(Icons.error_outline),
                  SizedBox(width: 10,),
                  Text('접종명 클릭 시 접종 주사에 대한 설명을 볼 수 있습니다.'),
                ],
              ),
              Row(
                children: const [
                    ...
                    content: Container(
                      ...
                      child: Column(
                        children: [
                          ...
                          Text(vaccine.recommendationDate, style: TextStyle(fontSize: 14)),
                          Padding(
                                    ...
                                    child: Padding(
                                        ...
                                        child: Column(
                                          children: [
                                            ...
                                            Text('미접종', style: TextStyle(color: Colors.grey),)
                                          ],
                                        )
                                    ),
                                  ),
                                      ...
                                      child: Padding(
                                          ...
                                          child: Column(
                                            children: [
                                              ...
                                              Text('접종', style: TextStyle(color: Colors.blue),)
                                            ],
                                          )
                                      )
                                  )
                          ...
                          Container(
                              ...
                              child: ElevatedButton(
                                ...
                                onPressed: (){
                                  if(isSelected[1]){
                                    setVaccineInfo(vaccine.ID, 'Finish');
                                  }
                                  Get.back();
                                },
                                child: const Text('확인')
  ...
  setVaccineInfo(int mode, String state) async{
    var r = [widget.baby.relationInfo.BabyId,'check name', mode, state];
  }
  Future getMyVaccine() async{
    var vaccineList = await vaccineCheckById(widget.baby.relationInfo.BabyId);
  }
}
